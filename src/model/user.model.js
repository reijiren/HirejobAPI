...
const userModel = {
    ...
    findWorkers: (skill, page, limit, order) => {
        return new Promise((resolve, reject) => {
            const offset = (page - 1) * limit;
            
            db.query(`
            select * from workers join users on users.id = workers.id where skill ilike '%${skill}%' order by ${order} asc limit ${limit} offset ${offset};
            `, (err, res) => {
                if(err) return reject(err);
                resolve(res);
            })
    ...
    updateWorkers: (id, data, pass) => {
        return new Promise((resolve, reject) => {
            ...
            with upd as (
                ...
                bg_photo = coalesce ($9, bg_photo),
                instagram = coalesce ($10, instagram),
                github = coalesce ($11, github),
                gitlab = coalesce ($12, gitlab),
                linkedin = coalesce ($13, linkedin),
                description = coalesce ($14, description)
                from workers
                where users.id = workers.id and users.id = $15
                returning *
            )
            update workers set
            job_type = coalesce ($4, workers.job_type),
            skill = coalesce ($7, workers.skill)
            from upd
            where workers.id = $15;
            `, [data.name, data.phone, pass, data.job_type, data.job_title, data.city, data.skill, data.photo, data.bg_photo, data.instagram, data.github, data.gitlab, data.linkedin, data.description, id]
            , (err, res) => {
                if(err) return reject(err);
                resolve(res);
            })
